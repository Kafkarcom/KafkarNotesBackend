...
CREATE TABLE user_note_shortcuts (
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  note_id UUID NOT NULL REFERENCES notes(id) ON DELETE CASCADE,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
CREATE INDEX sub_notes_search_idx ON sub_notes USING GIN(search_vector);

-- Note-graph cache for performance
CREATE TABLE note_graph_cache (
...
CREATE INDEX idx_sub_notes_note_id ON sub_notes(note_id);
CREATE INDEX idx_sub_notes_position ON sub_notes(position);
CREATE INDEX idx_sub_note_history_sub_note_id ON sub_note_history(sub_note_id);
...
CREATE INDEX idx_note_permissions_email ON note_permissions(email);

-- Create GIN index for metadata JSONB
CREATE INDEX idx_notes_metadata ON notes USING GIN(metadata);
CREATE INDEX idx_sub_notes_content ON sub_notes USING GIN(content);
CREATE INDEX idx_users_preferences ON users USING GIN(preferences);

-- Create index for case-insensitive searches
CREATE INDEX idx_notes_title_lower ON notes(LOWER(title));
